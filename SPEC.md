...
- `--lang <language>`: `auto`, `js`, `ts`, `php`, `python`, or `go`; default `auto`
- `--framework <name>`: `auto`, `next`, `react`, `express`, `wordpress-plugin`, `fastapi`, or `none`; default `auto`
...
- language from `tsconfig.json`, `package.json`, `composer.json`, WordPress plugin headers, `go.mod`, `pyproject.toml`, or `requirements.txt`
- framework from package dependencies, WordPress headers, or common Python project files
...
- Supports WordPress plugin/theme analysis including plugin headers, Composer PSR-4 autoload, hooks, AJAX nonces, capabilities, wpFluent tables, Vue components, Vite, and context gaps.
- Writes `.claude/context-map.md`, `.claude/context-gaps.json`, and `.claude/summaries/*.md`.
...
- `.claude/context-map.md` when generated by `cco init --analyze`
- `.claude/context-gaps.json` when generated by `cco init --analyze`
- `.claude/summaries/backend.md` when generated by `cco init --analyze`
- `.claude/summaries/frontend.md` when generated by `cco init --analyze`
...
Safely scans and classifies repository files without writing files.

Options:

- `--json`: print machine-readable JSON
- `--max-files <n>`: maximum files to classify; default `400`

Scanner behavior:

- skips dependency, VCS, build, cache, generated asset, virtualenv, secret-like, and oversized files
- classifies files by role and language
- returns grouped file lists by role and language
...
- `--cache`: write `.cco/cache/analysis.json`
...
- context model with project, entrypoints, symbols, graph, and context gaps
