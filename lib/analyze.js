...
function detectPathGroups(files, configFiles) {
  ...
  return {
    apiDirs: [
      ...
      hasPrefix("app/Http/Controllers/") ? "app/Http/Controllers" : null,
      hasPrefix("server/") ? "server" : null,
      hasPrefix("routes/") ? "routes" : null,
    ].filter(Boolean),
    componentDirs: [
      hasPrefix("src/components/") ? "src/components" : null,
      hasPrefix("resources/js/components/") ? "resources/js/components" : null,
    ...
    databaseDirs: [
      ...
      hasPrefix("app/Models/") ? "app/Models" : null,
...
export async function analyzeRepository(cwd = process.cwd(), options = {}) {
  ...
  const context = {
    project: {
      ...
      type: profile,
      purpose: pluginHeader?.Description || themeHeader?.Description || pkg?.description || composer?.description || null,
    ...
    gaps: [
      ...
      ...(!contextPurpose(pluginHeader, themeHeader, pkg, composer) ? [{ field: "project.purpose", question: "Add a one-sentence project purpose.", confidence: 0.4 }] : []),
    ],
  };
...
function contextPurpose(pluginHeader, themeHeader, pkg, composer) {
  return pluginHeader?.Description || themeHeader?.Description || pkg?.description || composer?.description || null;
}
