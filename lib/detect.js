...
async function hasWordPressPluginHeader(cwd) {
  const entries = await fs.readdir(cwd);
  for (const entry of entries) {
    if (!entry.endsWith(".php")) continue;
    const content = await fs.readFile(path.join(cwd, entry), "utf8");
    if (content.includes("Plugin Name:")) return true;
  }
  return false;
}
...
export async function detectProject(cwd = process.cwd(), options = {}) {
  ...
  const isWordPressPlugin = composer?.type === "wordpress-plugin" || await hasWordPressPluginHeader(cwd);

  let language = "ts";
  if (hasGoMod) language = "go";
  else if (hasPyProject || hasRequirements) language = "python";
  else if (hasComposer || isWordPressPlugin) language = "php";
  else if (pkg && !hasTsConfig) language = "js";
  else if (hasTsConfig) language = "ts";

  let framework = detectFrameworkFromPackage(pkg);
  if (isWordPressPlugin) framework = "wordpress-plugin";
  ...
  else if (hasComposer && pkg) packageManager = "composer+npm";
  else if (hasComposer) packageManager = "composer";
  ...
  const packageRunner = packageManager === "npm" || packageManager === "composer+npm" ? "npm run" : `${packageManager} run`;
  const installCommand = {
    npm: "npm install",
    pnpm: "pnpm install",
    yarn: "yarn install",
    bun: "bun install",
    "composer+npm": "composer install && npm install",
  ...
  const commands = {
    ...
    test: pkg?.scripts?.test ? `${packageRunner} test 2>&1 | tail -n 100` : language === "go" ? "go test ./..." : language === "php" ? "" : "npm test 2>&1 | tail -n 100",
